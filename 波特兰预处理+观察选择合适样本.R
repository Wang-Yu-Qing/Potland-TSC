raw_data<-data.frame(raw_data[with(raw_data,detectorid==1945|detectorid==1946|detectorid==1947),])   #subset detectors
...
time<-rep(time,times=61*3)
date<-rep(date,each=4320)
date<-rep(date,time=3)
detectorid<-rep(c(1945,1946,1947),each=61*4320)
...
data1<-raw_data[raw_data$detectorid==1945,]
data2<-raw_data[raw_data$detectorid==1946,]
data3<-raw_data[raw_data$detectorid==1947,]
...
for (i in 1:nrow(new_data)) {if(is.na(new_data$speed[i]))
{new_data$speed[i]<-round(runif(1,1,5),digits = 0)}}                                               #aggregation

rm(data1,data2,data3,empty,raw_data)
...
for (i in 1:288) {volume[i]<-sum(day_data$volume[(i*15-14):(i*15)])*12/3}
