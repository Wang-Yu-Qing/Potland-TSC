...
p_value_s<-sum(sapply(c(2:15),function(k) multi_series[[i]][,2][k]-multi_series[[i]][,2][k-1]))
p_value_o<-sum(sapply(c(2:15),function(k) multi_series[[i]][,3][k]-multi_series[[i]][,3][k-1]))
if (-15>p_value_s&p_value_o>1.5) (multi_state[i]<-5
) else (if (p_value_s>10&-0.2>p_value_o) (multi_state[i]<-6
